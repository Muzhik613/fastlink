...
- `xai.mjs` — one Anthropic-Messages client for xAI's `/v1/messages`, reached **directly** when `XAI_API_KEY` is set (`XAI_BASE_URL`, default `https://api.x.ai`, `Authorization: Bearer <key>`) or else through the **grokcode proxy** (`GROKCODE_URL`, which injects the grok CLI login token from `$HOME/.grok/auth.json`; started if nothing answers). Either way the request gets the fixups xAI needs (`shapeRequest`: `required:[]` on object tool schemas, no `stop`/`stop_sequences`, `reasoning_effort` from `GROKCODE_EFFORT`).
