...
- **End-of-run visual note** (`visualNoteRound` in `runner.mjs`) — a write nothing could read back (a result saying `verified:false`, or a forced `fast_type` whose bypassed guard IS the missing read-back) is invisible to the evidence gate: a cross-origin iframe like Azure's portal blade cannot be re-read by any tool we have. So when `report_done` would be accepted and `unverifiedWrites(toolLog)` is non-empty, the runner takes **one** screenshot, asks the vision tier (`describeScreen`, the same Gemini plumbing as `fast_point`) what is ON the screen, and hands the model those plain observations plus an invitation — "anything you want to fix, or is that expected?". The note is deliberately **dumb**: it states what is visible and never classifies widgets, names a tool, diagnoses a cause or issues a verdict; the model owns the judgement. Acting on it and explaining why the screen is expected are both accepted, and the next `report_done` goes through either way. **One round per run, and it is the note's OWN round**: `reportDecision` runs the note BEFORE the evidence gate and outside the gate's refusal budget, because the note is the only check with eyes while the gate can only re-argue what is already in the tool log (live: the gate refused twice on wording, the model rewrote an honest result three times, and the note never ran). Every outcome lands on the run row as `visualNote:{observations, unverified, model_response, actedAfter}` or `visualNote:{skipped:"no vision"|"no screenshot"|"nothing observed"|…}`, so the notes can be audited for whether they earn their cost. Gate mode `off` takes no screenshot.
