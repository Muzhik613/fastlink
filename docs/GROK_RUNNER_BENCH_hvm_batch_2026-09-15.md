...
Client `grok_runner`, 7 pass(es) over 6 tests, driven by `bench/hvm-run.sh` on the hvm rig (Xvfb + Chrome for Testing + unpacked fast-ext + local broker; grokcode proxy :8791). Rows since 2026-09-15T18:16:53.000Z.
...
Runtime per pass: pass 1 on 3b2c772 (grok-4.3, phase2): batch build; pass 2 on 7d43d8a: select hints skip autocompletes, ARIA aria-controls wall clock, runBridge deadline; pass 3 on bf55fda: suggestions signal, trail poll 0.5s; pass 4 on 99001f6: fast_click commits suggestion entries (mouse); pass 5 on 973cf06 = mapsdir ONLY (4.3); pass 6 on 973cf06 = grok-4.6/phase2 CONTROL; pass 7 on a490fe6 = mapsdir ONLY (4.3; keyCode keys, verified pick, fast_wait deadline); pass 7 on a490fe6 (grok-4.3, toolset phase2);

Run store per pass (model/toolset): pass 1 = grok-4.3/phase2; pass 2 = grok-4.3/phase2; pass 3 = grok-4.3/phase2; pass 4 = grok-4.3/phase2; pass 5 = grok-4.6/phase2 + grok-4.3/phase2; pass 6 = grok-4.6/phase2; pass 7 = grok-4.3/phase2.

## Per test

Cell = `score/total wall calls [m=model time, sum of turn latencies]` · flags: X invalid, S stuck, ! overclaim, ~ underclaim.

| test | pass 1 | pass 2 | pass 3 | pass 4 | pass 5 | pass 6 | pass 7 | best wall | median wall | median calls | score | baseline wall | best vs baseline |
|---|---|---|---|---|---|---|---|---:|---:|---:|---|---:|---:|
...
| mapsdir | 4/6 25.7s 9c m=11.7s ~ | 5/6 9.6s 7c m=13.4s ! | 4/6 36.8s 15c m=19.4s ~ | 5/6 93.8s 19c m=31.4s ! | 5/6 47.2s 13c m=23.1s ! | 6/6 28.3s 12c m=41.1s | 6/6 19.3s 7c m=13.6s | 9.6s | 28.3s | 12 | 4–6/6 | 41.9s | −32.3s (-77%) |
...
Totals over valid cells: 32 cells, 297/307 checkpoints, 194 tool calls, 460.0s wall.
...
|---|---:|---:|---:|---:|---|
| fast_snapshot | 55 | 0 | 50 | 0 | multipage, staticform, overlay, flightsearch, mapsdir, extract |
| fast_click | 36 | 8 | 518 | 9 | multipage, overlay, flightsearch, mapsdir |
| fast_tab | 32 | 0 | 351 | 0 | multipage, staticform, overlay, flightsearch, mapsdir, extract |
| fast_text | 22 | 1 | 24 | 2 | staticform, overlay, flightsearch, mapsdir, extract |
| fast_wait | 12 | 9 | 10092 | 9 | multipage, mapsdir |
| fast_batch | 11 | 0 | 2374 | 0 | staticform, flightsearch, mapsdir |
| fast_key_press | 10 | 0 | 29 | 0 | mapsdir |
| fast_fill | 9 | 0 | 168 | 0 | flightsearch, mapsdir |
| fast_select_option | 6 | 1 | 1344 | 0 | overlay, mapsdir |
| fast_scroll | 1 | 0 | 49 | 0 | flightsearch |

Total: 194 calls across 10 distinct tools, 20 fumbles.
...
- pass 7 mapsdir #6: `fast_wait` (text=route options) — failed → followed by fast_snapshot — {"error":"Timed out waiting for \"route options\"","settling":false,"sinceMutMs":2268,"sinceNetMs":2
