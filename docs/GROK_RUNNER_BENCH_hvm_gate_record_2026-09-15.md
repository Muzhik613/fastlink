...
Runtime per pass: 4.3 alone: gate record-only on 3839858 pass 1 on 3839858 (grok-4.3, toolset phase2, gate record); pass 2 on 4c2ee8f (grok-4.3, toolset phase2, gate record); pass 3 on b755b08 (grok-4.3, toolset phase2, gate record);

Run store per pass (model/toolset/gate): pass 1 = grok-4.3/phase2/gate record; pass 2 = grok-4.3/phase2/gate record; pass 3 = grok-4.3/phase2/gate record.
...
|---|---|---|---|---:|---:|---:|---|---:|---:|
| multipage | 6/6 5.1s 5c m=8.1s | 6/6 8.1s 7c m=11.7s | 6/6 6.5s 6c m=11.6s | 5.1s | 6.5s | 6 | 6–6/6 | 16.9s | −11.8s (-70%) |
| staticform | 12/12 8.2s 4c m=8.3s | 12/12 7.5s 4c m=9.5s | 12/12 8.9s 4c m=9.3s | 7.5s | 8.2s | 4 | 12–12/12 | 30.4s | −22.9s (-75%) |
| overlay | 3/3 7.3s 4c m=6.9s | 3/3 3.7s 3c m=5.6s | 3/3 3.6s 3c m=4.6s | 3.6s | 3.7s | 3 | 3–3/3 | 66.0s | −62.4s (-95%) |
| flightsearch | 10/10 11.0s 5c m=15.2s | 10/10 8.9s 4c m=10.8s | 10/10 9.0s 4c m=10.9s | 8.9s | 9.0s | 4 | 10–10/10 | 23.3s | −14.4s (-62%) |
| mapsdir | 6/6 7.6s 6c m=10.0s | 5/6 22.2s 7c m=14.2s ! | 6/6 8.5s 7c m=11.0s | 7.6s | 8.5s | 7 | 5–6/6 | 41.9s | −34.3s (-82%) |
| extract | 22/22 5.0s 3c m=9.6s | 22/22 4.9s 3c m=10.7s | 22/22 2.2s 2c m=8.5s | 2.2s | 4.9s | 3 | 22–22/22 | 5.7s | −3.5s (-61%) |

Totals over valid cells: 18 cells, 176/177 checkpoints, 81 tool calls, 138.2s wall.
...
- pass 3 overlay: scored 3/3 (full — the gate would have been WRONG) — no tool has read the page since your last fast_select_option; call fast_snapshot or fast_text (its own auto-snapshot is not a read-back) and cite what it return
...
- pass 3 flightsearch: scored 10/10 (full — the gate would have been WRONG) — your result says "opened" but no fast_click / fast_nav / fast_tab (beyond the first page load) call succeeded. If the task asked you to open, do it now; only if
...
|---|---:|---:|---:|---:|---|
| fast_snapshot | 29 | 0 | 95 | 0 | multipage, staticform, overlay, flightsearch, mapsdir, extract |
| fast_tab | 18 | 0 | 324 | 0 | multipage, staticform, overlay, flightsearch, mapsdir, extract |
| fast_click | 10 | 1 | 357 | 3 | multipage, overlay, mapsdir |
| fast_batch | 6 | 0 | 2657 | 0 | staticform, flightsearch |
| fast_text | 5 | 0 | 38 | 0 | flightsearch, mapsdir, extract |
| fast_wait | 4 | 1 | 2708 | 1 | multipage, mapsdir |
| fast_fill | 3 | 0 | 334 | 0 | mapsdir |
| fast_key_press | 3 | 0 | 35 | 0 | mapsdir |
| fast_select_option | 3 | 0 | 888 | 0 | overlay |

Total: 81 calls across 9 distinct tools, 4 fumbles.
