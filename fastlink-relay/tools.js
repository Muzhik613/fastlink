...
export const TOOLS = [
  ...
  {
    name: 'fast_profile',
    description: 'Pin WHICH browser this connection drives when more than one is connected. FastLink can be paired with several browsers at once (several Chrome profiles on the local broker; several paired browsers on the cloud relay), and each one has a NAME its owner sets on that browser\'s FastLink options page — the broker slot label ("primary", "secondary", or a custom label) locally, "This browser\'s name" on the cloud relay. Call once: install:"<name>" pins EVERY later call from this connection to that browser; the pin is held server-side and survives reconnects and clients that open a fresh session per tool call. install:"auto" releases it (calls then go to the most recently connected browser). If the pinned browser is not connected, calls fail with a clear error naming which browsers ARE connected — they are NEVER silently redirected to a different browser. fast_status lists the paired browsers, which are connected, and the current selection.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'fast_fill',
    description: 'Fill input/textarea/contenteditable by label/placeholder/name. An EXACT label/aria-label/placeholder/name match is preferred over a substring match, so "Email" won\'t lose to "Email confirmation". Replaces existing value by default; pass append: true to keep and add to existing. When the same label repeats, disambiguate with `index` (N-th match in document order) or `section` (scope to one titled group; `near` is an alias for it). `section` NEVER falls back: if no heading/legend on the page matches it, or the resolved section holds no such field, the call ERRORS and returns the page\'s `sections` list instead of filling a same-labelled field in a DIFFERENT section. Returns include a FRESH POST-fill `snapshot` (opt out with noSnapshot:true): a re-walk taken AFTER the fill settles, so a post-fill re-render IS reflected — chain off it without a separate fast_snapshot (falls back to the pre-fill capture, flagged `snapshotStale:true`, only if the page is too heavy to re-serialize). On very large pages a miss\'s diagnostics may be partial/count-only (heavy pages are bounded, not frozen). The input/change events are dispatched composed:true, so they cross shadow boundaries and validate correctly on web-component / Angular-Material / custom-element design systems. **The result LEADS with `verified` and `value`** — the field\'s LIVE value after the page settled — so a value the page reformatted or reverted is visible (verified:false carries a `reason`); report `value`, not what you sent. A miss keeps looking for the field for up to 1.5s before returning `candidates` (visible fillable fields), `hiddenMatches` + `hint`, and `settling:true` when the page was still changing — retry with a name from candidates, or open the control that reveals the field first.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        section: { type: 'string', description: 'Scope the field match to one titled group, e.g. "Authorized redirect URIs". Use when the same label appears in multiple sections. Sections are resolved by DOCUMENT OUTLINE (the group runs from its heading/legend to the next heading of the same or higher level), so it works even when the heading is not an ancestor of the fields (Angular Material / GCP) and is not confused by per-row sub-headings. If the section cannot be resolved, or contains no matching field, the call ERRORS and lists the page\'s section titles — it never silently fills a field elsewhere.' },
        near: { type: 'string', description: 'Alias for `section` (same resolver, same hard error on a miss). Prefer `section`.' },
  ...
  {
    name: 'fast_fill_form',
    description: 'Fill multiple fields in one call (saves N round-trips for large forms). Pass a { fields } object where keys are label/placeholder/name/aria-label substrings and values are the strings to fill. ALSO sets native <select> dropdowns: when a matched field is a <select>, the value is matched against option TEXT or VALUE and the option is selected (not typed) — so for a form with many dropdowns + inputs you can fill it ALL in one call. (Custom react-select/ARIA dropdowns are not native <select>s — use fast_select_option, which has a batch `selections` map, for those.) A field value may instead be an object { value, name, index, exact, section } to disambiguate collisions: `name` (or `exact:true`) matches the input\'s exact `name` attribute instead of a substring, `index` (0-based, N-th match in document order) picks which field when labels/names repeat (e.g. two product slots), and `section` scopes that field to one titled group (same resolver and same hard error as fast_fill — an unresolvable section fails THAT field loudly instead of writing a same-labelled field in another section). A top-level `section` applies to every field unless a field overrides it. Returns a per-field results map showing which were filled and which were not found, plus a FRESH POST-fill `snapshot` (opt out with noSnapshot:true): a re-walk taken AFTER the fills settle, so a post-fill re-render IS reflected (falls back to the pre-fill capture, flagged `snapshotStale:true`, only if the page is too heavy to re-serialize). A field filled while not visible (display:none/no offsetParent) but still submittable is flagged `filledButNotVisible:true`. Every field is re-read after the page settles: each per-field result carries `value` (live) and `verified`; a value an async AJAX re-render wiped is `reverted:true`, a top-level `reverted` array lists those keys, and the top-level `verified` (first field) is true only when every field held. Value-change events (input/change) are dispatched composed:true, so fields validate correctly across shadow boundaries on web-component / Angular-Material / custom-element design systems.',
    inputSchema: {
      type: 'object',
      properties: {
        fields: { type: 'object', description: 'Map of match-string → value, e.g. { "email": "...", "phone number": "...", "country": "US" }. Each key is matched case-insensitively against label/placeholder/name/aria-label. A value may also be an object { value, name, index, exact, section }: `name` matches the field\'s exact `name` attribute, `exact:true` matches the key as an exact name, `index` (0-based) selects which of multiple matches to fill in document order, and `section` scopes it to one titled group.' },
        section: { type: 'string', description: 'Scope EVERY field to one titled group (heading/legend), e.g. "Authorized redirect URIs"; a per-field { section } overrides it. `near` is an alias. Resolved by document outline; an unresolvable section fails those fields with an error listing the page\'s section titles rather than filling a same-labelled field elsewhere.' },
