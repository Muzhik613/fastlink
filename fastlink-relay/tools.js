...
export const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        frame: { type: 'string', description: 'Wait inside the visible cross-origin frame whose URL contains this (see `frames` / `framesNote` in fast_snapshot, or an ambiguity refusal\'s candidates). Omit to let the tool find the document that holds the target.' },
  ...
  },
  {
    ...
    description: 'Capture the active Chrome tab and return the IMAGE itself (you see it directly). Use it to SEE what DOM tools cannot read: the content of a cross-origin iframe (a snapshot\'s frameNotice names these), a canvas, an image, a custom-drawn control — then act on it with fast_click_xy / fast_type at coordinates read straight off this image. Also use it to confirm something looks right. For ordinary page text and controls prefer fast_snapshot: it is instant and exact. The image is at CSS-pixel scale: a point read off it IS the x,y fast_click_xy takes, one to one, on every screen (HiDPI included); a text item alongside carries {cssWidth, cssHeight, dpr, scale:1}. Pass fresh:true if a recent screenshot looked stale/identical after a focus/navigation change — it reads the live window surface via CDP instead of the compositor frame chrome.tabs.captureVisibleTab may re-serve.',
    inputSchema: {
      type: 'object',
      properties: {
