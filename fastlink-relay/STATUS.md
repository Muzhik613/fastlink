...
| Per-user DO (`userRelay.js`) | relay-core | ✅ WS hibernation, pending map, named multi-browser targeting (`resolveTarget`), `getScout()` |
...
- **Multi-browser NAMED targeting (SPEC §3d)**: `#acceptExtension` stamps each socket `serializeAttachment({connectedAt, deviceToken})` (and every later write MERGES, so the stamp is never erased); `devices.label` is a per-user-unique NAME (migration `0006`); `resolveTarget(clientKey)` maps selection → name → token → socket. `fast_profile install:"<name>"|"auto"` sets the selection, stored durably in the DO keyed by OAuth client id (survives token refresh AND clients that re-`initialize` on every tool call); a per-user default covers clients that never call it. An offline/unknown selection is a hard error naming the connected browsers — there is NO most-recent-wins fallback except in explicit `auto` mode. `fast_status` reports `browsers`, `selected`, `routedBrowser`, `devicesConnected`. Names are edited from the extension options page via device-token-authed `GET/POST /devices`.
